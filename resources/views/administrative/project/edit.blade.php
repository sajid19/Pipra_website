...
<div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
    <div>
        <h4 class="mb-3 mb-md-0"> Project Edit</h4>
    </div>
...
<div class="row">
    <div class="col-md-8 grid-margin stretch-card offset-md-2">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">Project Edit</h6>
                <form class="forms-sample" action="{{route('administrative.project.update',$data->id)}} " method="POST" enctype="multipart/form-data">
                    @csrf
                    {!! method_field('PUT') !!}
                    <div class="form-group {{ $errors->has('serial') ? 'has-danger' : '' }}">
                        <label for="serial">Serial</label>
                        <input value="{{$data->serial}}" required type="text" class="form-control form-control-danger" id="serial" name="serial" autocomplete="off" placeholder="Serial" aria-invalid="true">
                    ...
                    <div class="form-group {{ $errors->has('title') ? 'has-danger' : '' }}">
                        ...
                        <input required type="text" value="{{$data->title}}" class="form-control form-control-danger" id="title" name="title" autocomplete="off" placeholder="Title" aria-invalid="true">
                        @if($errors->has('title'))
                    ...
                    <div class="form-group {{ $errors->has('sub_title') ? 'has-danger' : '' }}">
                        ...
                        <textarea type="text" class="form-control form-height @error('sub_title') border @enderror" id="sub_title" rows="4" name="sub_title">{!!$data->sub_title!!}</textarea>
                    ...
                    <div class="form-group {{ $errors->has('description') ? 'has-danger' : '' }}">
                        ...
                        <textarea type="text" class="form-control form-height @error('description') border @enderror" id="description" rows="8" name="description">{!!$data->description!!}</textarea>
                    ...
                    <button type="button" class="btn btn-primary mb-3" id="add-new-section">Add Slider Images</button>
                    <div id="slider-images-container" data-row="{{$sliders? $sliders->count() + 1:1}}">
                        @if($sliders)
                        @foreach($sliders as $img)
                        <div class="image-section">
                            <h5>Slider image {{$loop->iteration}}</h5>
                            <hr>
                            <div class="row">
                                <div class="col-md-9">
                                    @if(!empty($img->file))
                                    <div class="mb-3">
                                        <img src="{{ $img->file }}" alt="" data-lazyload="{{ $img->file }}" height="80" width="80">
                                    </div>
                                    @endif
                                </div>
                                <div class="col-md-3">
                                    <button type="button" class="btn btn-danger btn-remove-section" data-id="{{$img->id}}" style="margin-top:32px">Remove</button>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    $(document).on('click', '.btn-remove-section', function() {
        ...
        if (id) {
            var url = "{{route('administrative.slider.image.project.destroy')}}";
            $.get(url, {
                id: id,
            }, function(data) {
